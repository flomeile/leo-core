...
| `00_INDEX\scripts\version-check.ps1` | Versionsprüfung ohne Modell: eingespielte Version gegen den neuesten Tag auf GitHub, eine Zeile Ausgabe; die Grundpflege in `AGENTS.md`, Abschnitt 1 ruft es zu Sessionbeginn, wenn der letzte Lauf über sieben Tage zurückliegt (seit 3.10). Der Zeitstempel `10_System\version-check-last-run.txt`, den es schreibt, gehört dir und steht in der `.gitignore` |
...
| `10_System\Google-Anbindung.md` | Einmalige Einrichtung des Google-Zugangs (Cloud-Projekt, APIs, Nutzertyp, OAuth-Client) und Kurzfassung der Mailregeln (seit 3.4) |
